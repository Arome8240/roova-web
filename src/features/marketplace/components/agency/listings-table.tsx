...
import { useMarkListingSold } from "@/features/marketplace/mutations";
...
export function ListingsTable() {
  ...
  const own = data?.filter((listing) => listing.agencyName === CURRENT_AGENCY_NAME) ?? [];
  const filtered = own.filter((listing) => filter === "all" || listing.status === filter);
    ...
    <div className="space-y-4">
      ...
      {data && filtered.length === 0 && (
        <EmptyState
          ...
          description="Try a different filter, or add a new listing."
        />
      )}
                  ...
                  return (
                    <tr key={listing.id} className="border-b border-border last:border-0">
                      ...
                      <td className="px-5 py-4 text-muted-foreground">
                        {listing.allowsInstallment ? `${listing.downPaymentPct}% down` : "Full only"}
                      </td>
                      <td className="px-5 py-4">
                        {listing.status === "live" && (
                          <Button
